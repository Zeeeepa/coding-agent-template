...
test.describe('Full Lifecycle Test', () => {
  test('complete workflow: select repo, create task, implement feature', async ({ page }) => {
    ...
    while (await loadingIndicator.isVisible() && loadingAttempts < 12) { // Wait up to 2 minutes
    ...
    const repoSelector = page.locator('[data-testid="repo-selector"]').or(
      page.locator('select').or(
        page.locator('input[placeholder*="repo"]').or(
          page.locator('button:has-text("Select")').or(
    ...
    const taskForm = page.locator('form').or(
      page.locator('[data-testid="task-form"]').or(
    ...
    const taskDescription = 'Add a new feature: implement a simple health check endpoint at /api/health that returns {"status": "ok", "timestamp": new Date().toISOString()}';
    
    // Try different selectors for the task input
    const taskInput = page.locator('textarea').or(
      page.locator('input[placeholder*="task"]').or(
        page.locator('input[placeholder*="describe"]').or(
          page.locator('[data-testid="task-input"]')
        )
      )
    );
    ...
    const agentSelector = page.locator('select').or(
      page.locator('[data-testid="agent-selector"]').or(
        page.locator('button:has-text("Claude")').or(
          page.locator('button:has-text("Cursor")')
        )
      )
    );
    ...
    if (await agentSelector.first().isVisible()) {
      ...
      const claudeOption = page.locator('option:has-text("Claude")').or(
    ...
    if (await submitButton.first().isVisible()) {
      ...
      const taskPage = page.locator('[data-testid="task-page"]').or(
        page.locator('h1:has-text("Task")').or(
          page.locator('.task-status').or(
            page.locator('[data-testid="task-status"]')
          )
        )
      );
      ...
      while (attempts < maxAttempts) {
        ...
        const completedIndicator = page.locator(':has-text("completed")').or(
          page.locator(':has-text("success")').or(
            page.locator(':has-text("finished")').or(
              page.locator('.status-success')
            )
          )
        );
        
        const errorIndicator = page.locator(':has-text("error")').or(
          page.locator(':has-text("failed")').or(
